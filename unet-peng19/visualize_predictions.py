...
class ToTensor:
    def __call__(self, sample):
        ...
        img1 = img1.numpy().transpose((0, 3, 1, 2))
        img2 = img2.numpy().transpose((0, 3, 1, 2))
...
class ConcatFusion:
    def __call__(self, sample):
        img1, img2, mask = sample
        image = torch.cat([img1, img2], axis=1)
        return image, mask
...
test_ids = np.arange(len(test_dataloader))
num_samples = 5
sampled_ids = np.random.choice(test_ids, size=num_samples, replace=False)

fig = plt.figure(figsize=[100, 40])
fig, axes = plt.subplots(nrows=num_samples, ncols=4)
...
for i, sample in enumerate(test_dataloader):
    if i in sampled_ids:
        ...
        with torch.no_grad():
            ...
            test_images, test_masks = test_transforms(sample)
            ...
            test_preds_flat = test_preds.view(-1).round().int().cpu()
            test_masks_flat = test_masks.view(-1).cpu()
            test_iou = jaccard_score(test_preds_flat, test_masks_flat)
            test_accuracy = accuracy_score(test_preds_flat, test_masks_flat)
            test_precision = precision_score(test_preds_flat, test_masks_flat)
            test_recall = recall_score(test_preds_flat, test_masks_flat)
            test_f1_score = f1_score(test_preds_flat, test_masks_flat)
            ...
            test_preds = test_preds.round().int()
            img1, img2, mask = torch.squeeze(img1), torch.squeeze(img2), torch.squeeze(mask)

        axes[n, 0].imshow(img1.cpu())
        axes[n, 0].axis("off")
        if n == 0:
            axes[n, 0].set_title("Image 1")

        axes[n, 1].imshow(img2.cpu())
        axes[n, 1].axis("off")
        if n == 0:
            axes[n, 1].set_title("Image 2")

        axes[n, 2].imshow(mask.cpu())
        axes[n, 2].axis("off")
        if n == 0:
            axes[n, 2].set_title("Ground truth")

        axes[n, 3].imshow(test_preds.cpu())
        axes[n, 3].axis("off")
        if n == 0:
            axes[n, 3].set_title("Prediction")
